# Geliştirme ve işletim rehberi

Bu belge kurulumun, ortam değişkenlerinin, migration'ların, Railway
dağıtımının ve yedeklemenin ayrıntılı rehberidir. Projenin tanıtımı için
[README](../README.md) sayfasına bakın.
...
kullanır. Ayrıntı: [docs/ARCHITECTURE.md](ARCHITECTURE.md)
...
### 3.1 Depoyu alın ve bağımlılıkları kurun
...
### 3.4 Migration'ı uygulayın ve bağlantıyı doğrulayın
...
### 3.5 İlk işletme sahibini oluşturun
...
### 3.6 Demo verisi (isteğe bağlı, yalnız ayrı bir veritabanında)

Uygulamayı dolu hâliyle görmek için **ayrı** bir demo veritabanı oluşturup örnek
veri yükleyebilirsiniz (ADR-024). Komut yalnız adı `demo` içeren ve boş olan bir
veritabanına yazar; gerçek işletme veritabanında çalışmayı reddeder.

```powershell
& "C:\Program Files\PostgreSQL\15\bin\createdb.exe" -U postgres KafeAdisyonDemo
$env:DATABASE_URL = "postgresql://postgres:PAROLANIZ@localhost:5432/KafeAdisyonDemo?schema=public"
npm run db:migrate:deploy
npm run demo:seed
npm run dev
```

Üretilenler: 5 kategori ve seçenekli ürünler, 3 salon ve 18 masa, 5 personel,
son 30 günün yaklaşık 550 satışı, açık masalar, mutfak siparişleri, açık kasa ve
geçmiş vardiyalar, cariler, reçeteli stok. Tüm demo hesaplarının şifresi
`Demo1234!`: `demo` (işletme sahibi), `elif` (kasiyer), `mert` (garson),
`mutfak` (mutfak). `DEMO_NOW` ile "bugün"ün saati sabitlenebilir
(ör. `2026-09-25T15:40:00+03:00`).

---

## 4. Development çalıştırma
...
### 7.1.1 Arayüzü ayrı barındırma (isteğe bağlı)
...
### 7.2 Tek servis ve custom domain
...
### 7.4 PostgreSQL backup ve restore
...
## 9. Mevcut veritabanı resetlenmez
...
| [docs/PHASES.md](PHASES.md)                                   | Phase 0–7 planı                         |
| [docs/ARCHITECTURE.md](ARCHITECTURE.md)                       | Mimari                                  |
| [docs/PRODUCT_SCOPE.md](PRODUCT_SCOPE.md)                     | Ürün kapsamı                            |
| [docs/UI_GUIDE.md](UI_GUIDE.md)                               | Arayüz rehberi                          |
| [docs/FINAL_ACCEPTANCE_REPORT.md](FINAL_ACCEPTANCE_REPORT.md) | Final review ve UAT kanıtları           |
